...
/* Creates a pad. If FIRST_PLAYER is passed as an argument, the pad for the first
 * player is created. if SECOND_PLAYER is passed as an argument, the pad for
 * the second player is created.
 *
 * @param place either FIRST_PLAYER or SECOND_PLAYER*/
Pad *pad_create(Uint8 place) {
	...
	SDL_SetColorKey(pad->sdl_image, SDL_SRCCOLORKEY | SDL_RLEACCEL, 2);
	if (place == FIRST_PLAYER) {
		pad->move_area.z1 = -40;
...
void puck_move(Puck * puck, Uint32 time) {
	...
	e = &puck->entity;
	if (e->x + e->dx * time < e->move_area.x1 + e->w / 2 || e->x + e->dx * time
			> e->move_area.x2 - e->w / 2)
